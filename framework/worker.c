...
int worker_handle_privmsg(struct worker_state *state, const struct api_msg *msg)
{
  ...
  sprintf(sql_stmt, "INSERT INTO Messages (sender, receiver, message) VALUES(\'%s\', \' %s\', \'%s\')", state->curruser, receiver, privmsg);
...
static int execute_request(struct worker_state *state, const struct api_msg *msg)
{
  ...
  case C_PRIVMSG:
  {
    return worker_handle_privmsg(state, msg);
  }
  case C_PUBMSG:
  {
    return worker_handle_pubmsg(state, msg);
  }
  case C_REGISTER:
  {
    return worker_handle_register(state, msg);
  }
  case C_USERS:
  {
    return worker_handle_users(state, msg);
  }
  case C_LOGIN:
  {
